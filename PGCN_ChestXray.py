...
# it under the terms of the GNU2000 General Public License as published by
...
import argparse  # to handle the arguments required for various function
...
def main(idx, lr, trial, prediction, dp):
    ...
    parser.add_argument('--lrate2', default=lrate, type=float, help='Initial learning rate (default: 0.005)')
    ...
    params['hidden2'] = args.hidden2  # Number of units in hidden layers
    ...
    graph2 = reader.get_genderGraphGen() # chestXray8
    ...
    #pathToSave = '/home/leslie/Desktop/arvind/MultiPGCN_ABIDE-master/CrossValidation/CXRResults_TrainableFalse300epochlr0.05AgeGraph3Classes/1_ALLLIST_Regularizeall_Lrate_e05_ExponetialDecay_1200epoch_feat2000_dropout02_2_' + str(trial)+'/'
    #pathToSave = '/home/campmortis/Desktop/AneesGCN_Results/ChinaSetChestXray/2March/numPara/WeightForRandomGraph500/StratifiedCrossValShuffledTrainableTrue500rawfeatures/GenderSITE_0505_'+ str(trial) + '/'
    pathToSave = '/home/leslie/Downloads/Parallel2br_chestxray-master/CrossValidation/TT_AgeGender_11_expDecay_Shuffle_TRY/' + str(lr) + '/oct11chestxray' + str(trial) + '/'
    if args.folds == 11:  # run cross validation on all folds

        #scores = Parallel(n_jobs=10)(
        #    delayed(train_fold)(train_ind, test_ind, test_ind, graph, graph2, features, y, y_data,
        #                        idx, lr, params, subject_IDs, pathToSave)
        #     for train_ind, test_ind in
        #     reversed(list(skf.split(np.zeros(num_nodes), np.squeeze(y)))))

        scores = [train_fold(train_ind, test_ind, test_ind, graph1, graph2, features, y, y_data,
                             idx, lr, params, subject_IDs, pathToSave, i) for train_ind, test_ind in
                  reversed(list(skf.split(np.zeros(num_nodes), np.squeeze(y)))) ]
        ...
        if args.save == 1:
            ...
            # Create a Pandas Excel writer using XlsxWriter as the engine.
            writer = pd.ExcelWriter(pathToSave + str(trial) + '.xlsx', engine='xlsxwriter')
            # Convert the dataframe to an XlsxWriter Excel object.
        ...
        scores_acc, scores_auc, scores_lin, scores_auc_lin, fold_size = train_fold(train, test, val, graph1, graph2,
                                                                                   features, y,
                                                                                   y_data, idx, lr, params, subject_IDs,
                                                                                   pathToSave,i)
